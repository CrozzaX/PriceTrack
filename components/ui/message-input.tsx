      ...
      <div className="relative flex w-full items-center space-x-2">
        <div className="relative flex-1">
          <textarea
            ...
            onKeyDown={onKeyDown}
            className={cn(
              "z-10 w-full grow resize-none rounded-xl border border-slate-200 bg-white p-4 pr-24 text-sm shadow-sm transition-all placeholder:text-slate-400 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50",
              showFileList && "pb-16",
              className
            )}
      ...
      <div className="absolute right-3 top-3 z-20 flex gap-2">
        {props.allowAttachments && (
          <Button
            ...
            onClick={() => {
              showFileUploadDialog().then((files) => {
                if (files) {
                  addFiles(files)
                }
              })
        ...
        <Button
          ...
          size="icon"
          className={cn(
            "h-8 w-8 rounded-full bg-primary text-primary-foreground transition-colors",
